...
class BipedLoco:
    ...
    def pos_callback(self, pos):
        ...
        for i, servo_id in enumerate(self.joints.keys()):
            pos = int(self.rl_init_positions[i])
            node.move_servo(servo_id, pos, speed, acc)


    def read_pos(self,servo_id):

        node = ST3215Driver()
        enc1 = node.read_position(servo_id)
        # print("enc1:", enc1)
        # enc11 = self.st.read_position(11)
        # enc12 = self.st.read_position(12)
        encoder = enc1
        print("encoder:", encoder)
        # print("enc1:", enc1[0], "enc2:", enc2[0], "enc3:", enc3[0], "enc4:", enc4[0], "enc5:", enc5[0], "enc6:", enc6[0], "enc7:", enc7[0], "enc8:", enc8[0], "enc9:", enc9[0], "enc10:", enc10[0])
        return encoder
    ...
    def stop(self):
        ...
        """
        for i, servo_id in enumerate(self.joints.keys()):
            self.driver.move_servo(servo_id, self.read_pos(servo_id))
