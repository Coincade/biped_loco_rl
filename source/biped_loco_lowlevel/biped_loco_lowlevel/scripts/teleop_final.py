...
# Left hand servo motors (leader) -> Left hand humanoid motors (follower)
LEFT_SERVO_MOTOR_IDS = [1, 2, 3, 5, 7, 9]
LEFT_HUMANOID_MOTOR_IDS = [1, 2, 3, 5, 7, 9]

# Right hand servo motors (leader) -> Right hand humanoid motors (follower)
RIGHT_SERVO_MOTOR_IDS = [2, 4, 6, 8, 10]
RIGHT_HUMANOID_MOTOR_IDS = [2, 4, 6, 8, 10]
...
def main():
    ...
    try:
        ...
        motor_bus_mapping = {
            ...
            for motor_id in ALL_HUMANOID_MOTOR_IDS
        }
        ...
        initial_motor_updates = []
        
        # Add left arm motors
        for servo_id, humanoid_id in zip(LEFT_SERVO_MOTOR_IDS, LEFT_HUMANOID_MOTOR_IDS):
            initial_motor_updates.append((humanoid_id, initial_servo_angles[servo_id]))
        
        # Add right arm motors
        for servo_id, humanoid_id in zip(RIGHT_SERVO_MOTOR_IDS, RIGHT_HUMANOID_MOTOR_IDS):
            # Apply sign inversion for right arm (except motor 4, based on teleop.py pattern)
            if servo_id == 4:
                initial_motor_updates.append((humanoid_id, initial_servo_angles[servo_id]))
            else:
                initial_motor_updates.append((humanoid_id, -initial_servo_angles[servo_id]))
